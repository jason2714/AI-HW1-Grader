...
if __name__ == '__main__':
    ...
    for cp in codePaths:
        ...
        if moduleName.endswith('_hw1'):
            student_id = moduleName.split('_')[0]
            moduleDict['search'] = loadModuleFile(moduleName, os.path.join(options.codeRoot, cp))
        else:
            moduleDict[moduleName] = loadModuleFile(moduleName, os.path.join(options.codeRoot, cp))
    moduleName = re.match('.*?([^/]*)\.py', options.testCaseCode).group(1)
    print moduleName
    print os.path.join(options.codeRoot, options.testCaseCode)
