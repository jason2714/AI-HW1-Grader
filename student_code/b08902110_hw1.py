...
def depthFirstSearch(problem):
    ...
    start = problem.getStartState() # vertex where the search starts
    if problem.isGoalState(start):
        return []

    stack = util.Stack()    # start with an empty stack
    visited = []
    stack.push((start, []))
    ...
    while not stack.isEmpty():
        ...
        if current_node not in visited:
            ...
            for next_node, next_actions, cost in neighbors:
                new_actions = actions + [next_actions]
    ...
    while not Queue.isEmpty():
        ...
        for next_node, action, cost in problem.getSuccessors(current_node):
            if not next_node in visited:
                ...
                Queue.push((next_node, actions + [action]))
    ...
    while not frontier.isEmpty():
        ...
        if not parent in visited:
            ...
            for child, action, cost in problem.getSuccessors(parent):
                frontier.push((child, actions + [action]), problem.getCostOfActions(actions + [action]))
    ...
    while not pQueue.isEmpty():
        ...
        if currentNode not in visited:
            ...
            for nextNode, action, cost in problem.getSuccessors(currentNode):
                pQueue.push((nextNode, actions + [action]),problem.getCostOfActions(actions + [action]) + heuristic(nextNode,problem))
